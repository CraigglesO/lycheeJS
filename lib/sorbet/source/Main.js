	...
	(function(projects) {
		...
		filesystem.dir('/lib').filter(function(value) {
			...
			if ((info1 !== null && info1.type === 'file')) {
				projects[id] = new sorbet.data.Project(id, '/lib/' + id);
			}
			...
			for (var id in settings.hosts) {
				...
				if (project === null) {
					...
					if (cache !== null) {
						...
						this.setHost(id, Object.values(_PROJECTS).filter(function(object) {
							var suffix = '/lib/' + object.identifier;
							return object.identifier === project || object.filesystem.root(-1 * suffix.length) === suffix;
						}));
	...
	Class.prototype = {
		...
		setHost: function(identifier, projects) {
			...
			if (identifier !== null) {
				...
				var cultivator = false;
				if (projects.length === Object.keys(_PROJECTS).length) {
					cultivator = true;
				}
