...
lychee.define('sorbet.mod.Server').requires([
	...
	var _serve = function(project, host, port) {
		...
		console.info('sorbet.mod.Server: BOOTUP ("' + project + '" | ' + host + ':' + port + ')');

		var server = _child_process.execFile(_root + project + '/sorbet.js', [
			_root,
			port,
			host
		], {
			...
			if (error) {
				console.error('sorbet.mod.Server: FAILURE ("' + project + '" | ' + host + ':' + port + ')');
			}
		...
		server.destroy = function() {
			...
			console.warn('sorbet.mod.Server: SHUTDOWN ("' + project + '" | ' + host + ':' + port + ')');
